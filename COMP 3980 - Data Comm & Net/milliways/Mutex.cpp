...
#define	Mutex_DefProc	DefWindowProc
BOOL Mutex_OnCreate (HWND hwnd, LPCREATESTRUCT lpCreateStruct);
void Mutex_OnDestroy (HWND hwnd);
void Mutex_OnCommand (HWND hwnd, int id, HWND hwndCtl, UINT codeNotify);
void Mutex_OnPaint (HWND hwnd);

static char szAppName[] = "Mutexes";
                          ...
                          WPARAM wParam, LPARAM lParam)
{
	switch (Message)
   {
   	HANDLE_MSG (hwnd, WM_CREATE, Mutex_OnCreate);
      HANDLE_MSG (hwnd, WM_DESTROY, Mutex_OnDestroy);
      HANDLE_MSG (hwnd, WM_COMMAND, Mutex_OnCommand);
      HANDLE_MSG (hwnd, WM_PAINT, Mutex_OnPaint);

      default:
      	return Mutex_DefProc (hwnd, Message, wParam, lParam);
   }
}
