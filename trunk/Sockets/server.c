...
void server(HWND hwnd)
{
	...
	/* Create TCP socket */
	if((ghSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET)
		MessageBox(NULL, "Unable to create TCP socket!", "ERROR", MB_OK);

	/* Set SO_REUSEADDR for port reuseability */
	if(setsockopt(ghSocket, SOL_SOCKET, SO_REUSEADDR, &arg, sizeof(arg)) == -1)
	...
	if(bind(ghSocket, (SOCKADDR *)&local, sizeof(SOCKADDR)) == SOCKET_ERROR)
		MessageBox(NULL, "Unable to bind address to socket!", "ERROR", MB_OK);

	/* Set max number of connections */
	if(listen(ghSocket, NUMCONNECTIONS))
		MessageBox(NULL, "Unable to set number of connections!", "ERROR", MB_OK);

	WSAAsyncSelect(ghSocket, hwnd, WM_SOCKET, FD_READ | FD_ACCEPT | FD_CLOSE);
}
