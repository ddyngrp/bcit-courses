...
int get_player_loc(unsigned long ip_netformat) {
	...
	for(pos = 0; pos < MAX_PLAYERS; pos++) {
		if(player_array[pos] != NULL && player_array[pos]->get_ip_netformat() == ip_netformat) { /* if this player's IP is the same as the IP we're searching for */
				return pos;
		}
	}
	
	return pos; /* should be MAX_PLAYERS if we've made it here */
}
