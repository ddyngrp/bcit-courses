...
int InitApp(HINSTANCE hInst, int nCmdShow) {
	...
	hEvent[0] = CreateEvent(NULL, FALSE, FALSE, TEXT("P1"));
	hEvent[1] = CreateEvent(NULL, FALSE, FALSE, TEXT("P2"));
	ghEvent = CreateEvent(NULL, FALSE, FALSE, TEXT("quit"));
	...
	while (loop) {
		switch (dwEvent) {
			case WAIT_OBJECT_0 + 0:
				szAppName = TEXT("PROGRAM ONE");
				loop = FALSE;
				break;
			case WAIT_OBJECT_0 + 1:
				szAppName = TEXT("PROGRAM TWO");
				loop = FALSE;
				break;
			case WAIT_ABANDONED:
				loop = FALSE;
