 ...
 * operations to service simultaneous connections to multiple client processes.
 * The server process creates a fixed number of pipe instances, each of which
 * can be connected to a separate client process. When a client process has
 * finished using its pipe instance, the server disconnects from the client
 * and reuses the pipe instance to connect to a new client.
*/
 ...
 * closes its handle to the pipe. Disconnect from this client, and then 
 ...
 * returns TRUE if an operation is pending or FALSE if the connection 
 * has been completed. 
 */ 
