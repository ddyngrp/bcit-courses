...
public class GuestInfo extends JPanel {
	...
	public GuestInfo(BedAndBreakfast db, JMenuItem guestNew, JMenuItem guestDelete,
		...
		guestDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    if (jlGuest.isSelectionEmpty()) {
			    	DialogBox.message("Nothing Selected to Delete.");
			    	return;
			    }
				deleteGuest();
			}
	...
	public void saveGuest() {		
		...
		try {
			Guest newGuest = new Guest(new Integer(newID + 1),
					jtGuestLName.getText(),
					jtGuestFName.getText(),
					jtGuestPhone.getText(),
					jtGuestEmail.getText(),
					new Address(	
			...
			db.getGuests().put(jtGuestLName.getText() + ", " + jtGuestFName.getText(),
					newGuest);
			
			listModel.addElement(jtGuestLName.getText() + ", " + jtGuestFName.getText());
			
			exportGuestXML();
		}
		catch (DataException ex) {
			DialogBox.message(ex.getData());
		}
	}
