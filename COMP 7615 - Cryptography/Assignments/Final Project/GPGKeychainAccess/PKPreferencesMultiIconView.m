...
@interface PKPreferencesMultiIconView (PKPreferecesMultiIconViewPrivate)
- (void)_focusFirst;
- (void)_focusLast;
...
@implementation PKPreferencesMultiIconView
...
- (void)mouseDown:(NSEvent *)event
{
	...
	if (index >= ([self _iconsWide] * (floor(eventLocation.y / buttonSize.height) + 1))) {
		return;
	}
	if (!NSWidth(buttonRect)) {
		return;
	}
...
- (void)keyDown:(NSEvent *)theEvent
{
	...
	if ([[theEvent charactersIgnoringModifiers] characterAtIndex:0] == kTabCharCode) {
		[self setKeyboardFocusRingNeedsDisplayInRect:[self _boundsForIndex:focusedIndex]];
		if (focusedIndex == NSNotFound && [self _numberOfIcons]) {
			focusedIndex = 0;
		...
		if (focusedIndex >= [self _numberOfIcons]) {
			if((nextView = [self nextValidKeyView])) {
				...
				focusedIndex = NSNotFound;
				if ([nextView isKindOfClass:[PKPreferencesMultiIconView class]]) {
					[(PKPreferencesMultiIconView *)nextView _focusFirst];
				}
			} else {
				focusedIndex = 0;
			}
		}
		if (![self _numberOfIcons]) {
			focusedIndex = NSNotFound;
		}
		...
		if ((signed) focusedIndex < 0 && [self _numberOfIcons]) {
			if ((nextView = [self previousValidKeyView])) {
				...
				focusedIndex = NSNotFound;
				if ([nextView isKindOfClass:[PKPreferencesMultiIconView class]]) {
					[(PKPreferencesMultiIconView *)nextView _focusLast];
				}
			} else {
				focusedIndex = [self _numberOfIcons] - 1;
			}
...
@implementation PKPreferencesMultiIconView (PKPreferencesMultiIconViewPrivate)
...
- (unsigned int)_numberOfRows
{
	...
	if(![self _column:&column andRow:&row forIndex:[self _numberOfIcons] - 1]) {
		return 0;
	}
	return row;
}
...
- (void)_drawIconAtIndex:(unsigned int)index drawRect:(NSRect)drawRect
{
	...
	if (!NSIntersectsRect(buttonRect, drawRect) ||
		![self _iconImage:&image andName:&name forIndex:index] ||
		![self _column:&column andRow:&row forIndex:index]) {
		return;
	}
...
//		[image drawInRect:destinationRect fromRect:nil operation:NSCompositeSourceOver fraction:1.];
	} else if (image) {
		NSImage *darkImage;
...
//		NSImage does not respond to drawFlippedInRect: operation: fraction:
//		[darkImage drawInRect:destinationRect fromRect:nil operation:NSCompositeSourceOver fraction:1.];
//		[image drawInRect:destinationRect fromRect:nil operation:NSCompositeSourceOver fraction:.6666667];
		[darkImage release];
	}
	if (focusedIndex == index) {
