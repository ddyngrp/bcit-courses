...
main(void)
{
	...
	/* was !startReceived */
	while (idCount != 2) /* While a client has not started the game */ 
	{
		read_fds = all_fds;
	...
	memcpy(playerBuf, serialize_player(player_array[1]), sizeof(DPlaya));
	for(j = 0; j <= nfds; j++)
	{
		if(FD_ISSET(j, &all_fds))
		{
		...
		for (i = 0; i <= nfds; i++)
		{
			if (FD_ISSET(i, &read_fds))
			{
				...
				else 						//Message from an already connected client
				{
					...
					memcpy(sendBuf, serialize_player(player_array[idCount]), sizeof(DPlaya));
					for(j = 0; j <= nfds; j++)
					{
						if(FD_ISSET(j, &all_fds))
						{
