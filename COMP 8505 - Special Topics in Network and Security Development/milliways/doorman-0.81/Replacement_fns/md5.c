...
#define FF(a, b, c, d, x, s, ac) \
  {(a) += F ((b), (c), (d)) + (x) + (__uint32_t)(ac); \
   (a) = ROTATE_LEFT ((a), (s)); \
   (a) += (b); \
  }
#define GG(a, b, c, d, x, s, ac) \
  {(a) += G ((b), (c), (d)) + (x) + (__uint32_t)(ac); \
   (a) = ROTATE_LEFT ((a), (s)); \
   (a) += (b); \
  }
#define HH(a, b, c, d, x, s, ac) \
  {(a) += H ((b), (c), (d)) + (x) + (__uint32_t)(ac); \
   (a) = ROTATE_LEFT ((a), (s)); \
   (a) += (b); \
  }
#define II(a, b, c, d, x, s, ac) \
  {(a) += I ((b), (c), (d)) + (x) + (__uint32_t)(ac); \
   (a) = ROTATE_LEFT ((a), (s)); \
   (a) += (b); \
  }
...
{
    mdContext->i[0] = mdContext->i[1] = (__uint32_t) 0;

    /* Load magic initialization constants.
   */
    mdContext->buf[0] = (__uint32_t) 0x67452301L;
    mdContext->buf[1] = (__uint32_t) 0xefcdab89L;
    mdContext->buf[2] = (__uint32_t) 0x98badcfeL;
    mdContext->buf[3] = (__uint32_t) 0x10325476L;
}
       ...
       unsigned int inLen)
{
    ...
    if ((mdContext->i[0] + ((__uint32_t) inLen << 3)) < mdContext->i[0])
        mdContext->i[1]++;
    mdContext->i[0] += ((__uint32_t) inLen << 3);
    mdContext->i[1] += ((__uint32_t) inLen >> 29);
    ...
    while (inLen--)
    {
        ...
        if (mdi == 0x40)
        {
            for (i = 0, ii = 0; i < 16; i++, ii += 4)
                in[i] = (((__uint32_t) mdContext->in[ii + 3]) << 24) |
                    (((__uint32_t) mdContext->in[ii + 2]) << 16) |
                    (((__uint32_t) mdContext->in[ii + 1]) << 8) |
                    ((__uint32_t) mdContext->in[ii]);
 ...
 */
void
Transform (register __uint32_t * buf, register __uint32_t * in)
{
    register __uint32_t a = buf[0], b = buf[1], c = buf[2], d = buf[3];
