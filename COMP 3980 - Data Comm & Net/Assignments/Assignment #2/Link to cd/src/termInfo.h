...
#define MAXCOLS		81	// limit on screen columns
...
#define SCROLLTOP(x)			(x.nScrollTop)
#define SCROLLBOTTOM(x)			(x.nScrollBottom)
...
#define OVERLAPPED(x)			(x.overlapped)

// Structures for Doug
struct _engInfo {
	TCHAR code[MAX_CODE_LENGTH];	// partial (or complete) vt100 code storage.
	int codeLen;	// number of code characters received.
	int mode;		// printing vs interpreting mode.
} engInfo;
