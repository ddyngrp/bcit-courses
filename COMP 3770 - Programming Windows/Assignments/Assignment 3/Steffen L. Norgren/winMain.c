...
// Private variables
TCHAR	szAppName[] = TEXT("Assignment #3");
...
int InitApp(HINSTANCE hInst, int nCmdShow) {
	...
	// Setup the program's canvas window class
	wc.lpfnWndProc		= (WNDPROC)CanvasProc;
	wc.hInstance		= hInst;
	wc.hCursor			= LoadCursor(NULL, IDC_CROSS);
	wc.hbrBackground	= (HBRUSH)(COLOR_WINDOW + 1);
	wc.lpszClassName	= TEXT("CanvasClass");
	...
	ghWndMain = CreateWindow(szAppName, szAppName,
		WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_BORDER | WS_VSCROLL | WS_HSCROLL,
