...
@interface PKPreferencesMultiIconView (PKPreferencesMultipleIconViewPrivate)
- (unsigned int)_numberOfRows;
@end

@interface PKPreferencesGroupedIconView (PKPreferencesGroupedIconViewPrivate)
...
@implementation PKPreferencesGroupedIconView
...
@implementation PKPreferencesGroupedIconView (PKPreferencesGroupedIconViewPrivate)
...
- (void)_generateGroupViewAtIndex:(unsigned int)index
{
	PKPreferencesMultiIconView *multiIconView = nil;
	NSString *identifier = [[preferencePaneGroups objectAtIndex:index] objectForKey:@"identifier"];
	NSString *name = NSLocalizedStringFromTable(identifier, @"PKPreferencePaneGroups", nil);
	...
	if (!(multiIconView = [self viewWithTag:index])) {
		...
		multiIconView = [[[PKPreferencesMultiIconView alloc] initWithFrame:[self _rectForGroupAtIndex:index]] autorelease];
		...
		[self addSubview:sep];
		if (index - 1 >= 0) {
			[[self viewWithTag:index - 1] setNextKeyView:multiIconView];
		}
		if (index == [self _numberOfGroups] - 1) {
			[multiIconView setNextKeyView:[self viewWithTag:0]];
		}
	}
