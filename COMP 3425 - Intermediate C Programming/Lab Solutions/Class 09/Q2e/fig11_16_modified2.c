...
double BonusDebit (FILE *fPtr, double DebitAmt);
   ...
   if ( ( cfPtr = fopen( "credit.dat", "rb+" ) ) == NULL ) {
      ...
      while ( ( choice = enterChoice() ) != 6 ) { 
         ...
         switch ( choice ) { 
			...
			/* update all records by adding debit amount */
            case 5:
				printf("\nThe total amount debited is: $%6.2f\n",BonusDebit (cfPtr, 10.00));
               break;
...
/* debit each account by debitAmt and 
            return total amount debited*/ 

double BonusDebit( FILE *fPtr, double debitAmt )
{ 
   int iCount = 0,  // used to count # of accounts in the file
	   iRecordNum = 0;  // tracks which record number 
                        // we are currently reading/writing
   ...
   /* read all records from random-access file, add 
   debit into text file and write record back to file                                       */
   while ( !feof( fPtr ) ) { 

	   /* read the record  */
	   fread( &client, sizeof( struct clientData ), 1, fPtr );
		 
	   if ( client.acctNum != 0 ) {  // if valid account

		   client.balance += debitAmt;  // update balance 
		                                // by adding debitAmt
		   iRecordNum = client.acctNum -1;  // record number 
		                            // that we have just read

		   /* move file pointer to correct record in file */
		   fseek( fPtr, ( iRecordNum ) * sizeof( struct clientData ), 
			   SEEK_SET );

		   /* write updated record over old record in file */
		   fwrite( &client, sizeof( struct clientData ), 1, fPtr );

		   // seek out next record
		   fseek( fPtr, ( ++iRecordNum ) * sizeof( struct clientData ), 
			   SEEK_SET );

		   ++iCount;  // increment count of accounts 
		              //  given bonus debit
   ...
   return ( debitAmt * iCount );  // return total amount debited
  
} /* end function BonusDebit */
...
{ 
   ...
   printf( "\nEnter your choice\n"
      ...
      "2 - update accounts by adding $10.00 debit\n"
      "3 - add a new account\n"
      "4 - delete an account\n"
	  "5 - update accounts by adding a $10.00 debit amount\n"
      "6 - end program\n? " );
